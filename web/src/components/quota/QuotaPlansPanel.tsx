...
export default function QuotaPlansPanel({ refreshKey = 0, onChanged }: { refreshKey?: number; onChanged: () => void }) {
  ...
  const load = () => api.quotaPlanStatistics().then(value => {
    setItems(value)
    setSelected(current => value.some(item => item.id === current) ? current : value[0]?.id || '')
  ...
  async function remove() {
    if (!draft?.can_delete || !window.confirm(t('admin:quotas.plans.deleteConfirm', { name: draft.name }))) return
    setBusy(true); setError('')
    try { await api.deleteQuotaPlan(draft.id); toast.success(t('admin:quotas.plans.deleted')); await load(); onChanged() }
    catch (reason) { setError((reason as Error).message) } finally { setBusy(false) }
  }
