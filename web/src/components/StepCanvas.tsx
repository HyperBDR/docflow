...
export default function StepCanvas({ step, onChange }: { step: Step; onChange: (values: Partial<Step>) => void }) {
  ...
  return <div>
    <div className="canvas-tools"><button className={mode === 'hotspot' ? 'selected' : ''} onClick={() => setMode('hotspot')}>{t('canvas.locateHotspot')}</button><button className={mode === 'redact' ? 'selected' : ''} onClick={() => setMode('redact')}>{t('canvas.drawRedaction')}</button>{step.redactions.length > 0 && <button onClick={() => onChange({ redactions: [] })}>{t('canvas.clearRedactions')}</button>}</div>
