...
export default function MonitoringDetailDrawer({ metricKey, ranges = defaultRanges, onClose }: { metricKey: MonitoringDetailKey; ranges?: string[]; onClose: () => void }) {
  ...
  return <><button className="admin-user-drawer-scrim" aria-label={t('common:actions.close')} onClick={onClose} /><aside className="admin-user-drawer admin-detail-drawer" role="dialog" aria-modal="true">
    <header className="admin-user-drawer-header"><div className="admin-detail-identity"><span className={value?.status || 'unknown'}><Icon name={item.icon} size={23} /></span><div><small>{t('details.eyebrow')}</small><h2>{t(item.title)}</h2><p><i className={value?.status || 'unknown'} />{t(`status.${value?.status || 'unknown'}`)}</p></div></div><button className="admin-user-drawer-close" aria-label={t('common:actions.close')} onClick={onClose}>×</button></header>
    ...
    <div className="admin-user-drawer-body admin-detail-body">{error && <div className="error">{error}</div>}{!value && !error && <div className="admin-detail-state-loading"><span className="action-spinner" />{t('loading')}</div>}{value && <>
      <section className="admin-detail-summary">{item.fields.map(key => { const formatted = displayValue(key, value.summary[key], locale); return <article key={key}><small>{t(`details.fields.${key}`, { defaultValue: key })}</small><strong><OverflowValue>{formatted}</OverflowValue></strong></article> })}</section>
      <section className="admin-detail-card monitor-history-card"><header><div><strong>{t('details.history')}</strong><small>{t('details.historyHint')}</small></div></header><InteractiveMetricChart points={chartPoints} series={item.series.map(series => ({ ...series, label: t(series.label) }))} unit={item.unit} threshold={threshold} /></section>
      {!!value.breakdown.length && <section className="admin-detail-card monitor-endpoint-card"><header><div><strong>{t('details.endpoints')}</strong><small>{t('details.endpointsHint')}</small></div></header><div className="monitor-endpoint-list admin-detail-card-scroll">{value.breakdown.slice(0, 10).map((row, index) => { const endpoint = `${String(row.method)} ${String(row.route)}`; return <article key={`${row.method}-${row.route}-${index}`}><OverflowValue className="monitor-endpoint-value">{endpoint}</OverflowValue><span><b>{Number(row.requests || 0).toLocaleString(locale)}</b>{t('details.requests')}</span><span><b>{Number(row.avg_latency_ms || 0).toLocaleString(locale)} ms</b>{t('details.averageLatency')}</span><span className={Number(row.error_rate || 0) >= 5 ? 'danger' : ''}><b>{Number(row.error_rate || 0)}%</b>{t('details.errorRate')}</span></article> })}</div></section>}
      <section className="admin-detail-card"><header><div><strong>{t('details.relatedAlerts')}</strong><small>{t('details.relatedAlertsHint')}</small></div></header>{value.alerts.length ? <div className="monitor-detail-alerts">{value.alerts.map(alert => <article key={alert.id}><SeverityBadge value={alert.severity} /><div><strong>{alert.title}</strong><small>{formatDate(alert.started_at, locale)}</small></div><span>{t(`alertStatus.${alert.status}`)}</span></article>)}</div> : <div className="admin-detail-state-empty">{t('details.noAlerts')}</div>}</section>
    </>}</div>
  </aside></>
}
