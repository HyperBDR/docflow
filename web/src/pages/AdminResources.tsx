...
export default function AdminResources() {
  ...
  }, [query, owner, status, contentLocale, page])
  ...
  return <div className="admin-content-page">
    ...
    <section className="admin-list-card">
      ...
      <div className="admin-pagination"><span>{t('pagination.range', { from: total ? (page - 1) * pageSize + 1 : 0, to: Math.min(page * pageSize, total), total })}</span><div><button disabled={page <= 1} onClick={() => setPage(value => value - 1)}><Icon name="chevronLeft" /></button><b>{page} / {pages}</b><button disabled={page >= pages} onClick={() => setPage(value => value + 1)}><Icon name="chevronRight" /></button></div></div>
    </section>
  </div>
}
