...
export default function PlatformSettings() {
  ...
  const [tab,setTab]=useState<'general'|'email'|'login'|'monitoring'>('general')
  return <main className="admin-content-page platform-settings-page"><div className="admin-page-intro"><div><h1>{t('title')}</h1><p>{t('subtitle')}</p></div></div><nav className="platform-settings-tabs">{([['general','settings'],['login','shield'],['email','message'],['monitoring','analytics']] as const).map(([key,icon])=><button key={key} className={tab===key?'active':''} onClick={()=>setTab(key)}><Icon name={icon}/><span><strong>{t(`tabs.${key}`)}</strong><small>{t(`tabs.${key}Hint`)}</small></span></button>)}</nav>{tab==='general'?<GeneralSettingsTab/>:tab==='email'?<EmailSettingsTab/>:tab==='login'?<GoogleAuthSettingsTab/>:<MonitoringSettingsTab/>}</main>
}
