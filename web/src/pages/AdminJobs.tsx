...
export default function AdminJobs() {
  ...
  return <div className="admin-content-page admin-jobs-page"><div className="admin-page-intro"><div><h1>{t('jobs.title')}</h1><p>{t('jobs.subtitle')}</p></div><span>{t('jobs.total',{count:total})}</span></div>
    ...
    {detailLoading&&!selected&&<div className="jobs-detail-loading"><span className="action-spinner"/></div>}{selected&&<><button className="admin-user-drawer-scrim" aria-label={t('common:actions.close')} onClick={close}/><aside className="admin-user-drawer job-detail-drawer" role="dialog" aria-modal="true" aria-labelledby="job-detail-title"><header className="admin-user-drawer-header"><div className="job-detail-identity"><span className={selected.job_type}><Icon name={selected.job_type==='ai'?'ai':'download'} size={22}/></span><div><h2 id="job-detail-title">{typeLabel(selected,t)}</h2><p>{selected.resource_title||t('overview.untitled')}</p><div><span className={`job-state ${selected.status}`}><i/>{t(`common:status.${selected.status}`)}</span><code>{selected.id}</code></div></div></div><button className="admin-user-drawer-close" onClick={close}>×</button></header><div className="admin-user-drawer-body job-detail-body">
        ...
        <section className="admin-user-section"><div className="admin-user-section-title"><span><Icon name="list"/></span><div><h3>{t('jobs.details.summary')}</h3><p>{t('jobs.details.summaryHint')}</p></div></div><div className="job-detail-grid detail-card-scroll"><div><small>{t('jobs.details.user')}</small><strong>{selected.user_name||selected.user_email||'—'}</strong><span>{selected.user_email}</span></div><div><small>{t('jobs.details.organization')}</small><strong>{selected.organization_name||'—'}</strong></div><div><small>{t('jobs.details.resource')}</small><strong>{selected.resource_title||'—'}</strong></div><div><small>{t('jobs.details.type')}</small><strong>{typeLabel(selected,t)}</strong></div><div><small>{t('jobs.details.model')}</small><strong>{selected.model||'—'}</strong></div><div><small>{t('jobs.details.progress')}</small><strong>{selected.progress}%</strong></div><div><small>{t('jobs.details.duration')}</small><strong>{duration(selected.duration_ms)}</strong></div><div><small>{t('jobs.details.retryOf')}</small><code>{selected.retry_of_id||'—'}</code></div></div></section>
        <section className="admin-user-section"><div className="admin-user-section-title"><span><Icon name="clock"/></span><div><h3>{t('jobs.details.timeline')}</h3><p>{t('jobs.details.timelineHint')}</p></div></div><div className="job-timeline detail-card-scroll">{[{key:'created',time:selected.created_at},{key:'started',time:selected.started_at},{key:selected.status==='cancelled'?'cancelled':selected.status==='failed'?'failed':'completed',time:selected.cancelled_at||selected.completed_at}].filter(item=>item.time).map(item=><div key={item.key}><i/><span><strong>{t(`jobs.timeline.${item.key}`)}</strong><small>{formatDate(item.time as string,locale)}</small></span></div>)}</div></section>
        {(selected.error||selected.error_code)&&<section className="admin-user-section job-error-section"><div className="admin-user-section-title"><span><Icon name="warning"/></span><div><h3>{t('jobs.details.error')}</h3><p>{selected.error_code||'—'}</p></div></div><pre className="detail-card-scroll">{selected.error||t('jobs.details.noErrorDetail')}</pre></section>}
        <section className="admin-user-section"><div className="admin-user-section-title"><span><Icon name="database"/></span><div><h3>{t('jobs.details.metadata')}</h3><p>{t('jobs.details.metadataHint')}</p></div></div><pre className="job-json detail-card-scroll">{JSON.stringify(selected.metadata,null,2)}</pre></section>
        {!!Object.keys(selected.result||{}).length&&<section className="admin-user-section"><div className="admin-user-section-title"><span><Icon name="check"/></span><div><h3>{t('jobs.details.result')}</h3><p>{t('jobs.details.resultHint')}</p></div></div><pre className="job-json detail-card-scroll">{JSON.stringify(selected.result,null,2)}</pre></section>}
      </div></aside></>}
  </div>
}
