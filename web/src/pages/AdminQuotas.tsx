...
export default function AdminQuotas() {
  ...
  const intervalSeconds = value?.interval_seconds || 300
  const intervalUnit = intervalSeconds % 3600 === 0 ? 'hours' : intervalSeconds % 60 === 0 ? 'minutes' : 'seconds'
  const intervalCount = intervalSeconds / (intervalUnit === 'hours' ? 3600 : intervalUnit === 'minutes' ? 60 : 1)
  const intervalLabel = t(`admin:quotas.intervalUnits.${intervalUnit}`, { count: intervalCount })
  return <main className="admin-content-page quota-operations-page"><div className="admin-page-intro"><div><h1>{t('admin:quotas.title')}</h1><p>{t('admin:quotas.subtitle')}</p></div><div className={`quota-collection-state ${value && !value.automatic_collection ? 'paused' : ''}`}><span><i />{t(value && !value.automatic_collection ? 'admin:quotas.automaticPaused' : 'admin:quotas.automatic', { value: intervalLabel })}</span>{value?.collected_at && <small>{t('admin:quotas.lastCollected', { date: formatDate(value.collected_at, locale) })}{value.next_collection_at && <><br />{t('admin:quotas.nextCollection', { date: formatDate(value.next_collection_at, locale) })}</>}</small>}<button className="icon-button" disabled={collecting} onClick={collect}>{collecting ? <span className="action-spinner" /> : <Icon name="analytics" />}{t(`admin:quotas.${collecting ? 'collecting' : 'collect'}`)}</button></div></div>
