...
export default function AdminResourceDetail() {
  ...
  useEffect(() => { setHotspotIndex(0) }, [index])
  ...
  const orderedHotspots = [...(step?.hotspots || [])].sort((a, b) => a.position - b.position)
  const sequential = step?.hotspot_mode === 'sequence'
  const atBeginning = index === 0 && (!sequential || hotspotIndex === 0)
  const atEnd = index >= steps.length - 1 && (!sequential || hotspotIndex >= orderedHotspots.length - 1)
  const previousPreview = () => {
    if (sequential && hotspotIndex > 0) { setHotspotIndex(value => value - 1); return }
    setIndex(value => Math.max(0, value - 1))
  }
  const nextPreview = () => {
    if (sequential && hotspotIndex < orderedHotspots.length - 1) { setHotspotIndex(value => value + 1); return }
  ...
  return <div className="admin-content-page resource-detail-page">
    ...
    {!governanceLoading&&tab==='shares'&&governance&&<ResourceShareAnalytics value={governance} locale={locale}/>} {!governanceLoading&&tab==='downloads'&&governance&&<ResourceDownloads value={governance} locale={locale}/>} {!governanceLoading&&tab==='audit'&&governance&&<ResourceAuditTrail value={governance} locale={locale}/>} {tab==='overview'&&<div className="resource-detail-layout">
      <section ref={previewRef} className="resource-preview-card"><header><span className={`status ${resource.status}`}>{t(`common:status.${resource.status}`)}</span><b>{t('resource.readOnly')}</b><small>{index + 1} / {steps.length}{sequential && orderedHotspots.length > 1 ? ` · ${hotspotIndex + 1} / ${orderedHotspots.length}` : ''}</small><button className="preview-fullscreen-button icon-button" onClick={togglePreviewFullscreen} title={t(previewFullscreen?'resource.governance.exitFullscreen':'resource.governance.fullscreenPreview')}><Icon name={previewFullscreen?'close':'layout'}/>{t(previewFullscreen?'resource.governance.exitFullscreen':'resource.governance.fullscreenPreview')}</button></header>
        <div className="admin-resource-stage">{step ? <SlideStage key={step.id} step={step} mode="player" fit="viewport" activeHotspotId={sequential ? orderedHotspots[hotspotIndex]?.id : undefined} theme={resource.demo.theme} navigation={resource.demo.navigation} stepIndex={index} stepCount={steps.length} onHotspot={nextPreview} onGuidePrevious={previousPreview} onGuideNext={nextPreview} /> : <div className="empty"><Icon name="image" size={32} /><p>{t('resource.noSteps')}</p></div>}</div>
        <footer><button disabled={atBeginning} onClick={previousPreview}><Icon name="chevronLeft" />{t('common:actions.previous')}</button><button disabled={atEnd} onClick={nextPreview}>{t('common:actions.next')}<Icon name="chevronRight" /></button></footer>
      </section>
