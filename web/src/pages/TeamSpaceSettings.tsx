...
export default function TeamSpaceSettings({ user, onUserChange, logout }: { user: User; onUserChange: (user: User) => void; logout: () => void }) {
  ...
  async function invite(event: React.FormEvent) {
    ...
    setBusy('invite'); setError(''); setMessage(''); setInviteUrl('')
    try { const result = await api.createInvitation(space.id, email, role); setInviteUrl(result.invite_url || ''); setEmail(''); setMessage(t('space.inviteCreated')) }
    catch (value) { setError(value instanceof Error ? value.message : t('common:errors.operationFailed')) }
    finally { setBusy('') }
  }
  ...
  async function updateMember(member: OrganizationMember, next: OrganizationRole) {
    ...
    try { const updated = await api.updateOrganizationMember(space.id, member.id, next); setMembers(list => list.map(item => item.id === updated.id ? updated : item)); setMessage(t('space.memberUpdated')) }
    catch (value) { setError(value instanceof Error ? value.message : t('common:errors.operationFailed')) }
    finally { setBusy('') }
  }
  ...
  async function removeMember(member: OrganizationMember) {
    ...
    setBusy(member.id); setError('')
    try { await api.removeOrganizationMember(space.id, member.id); setMembers(list => list.filter(item => item.id !== member.id)); setMessage(t('space.memberRemoved')) }
    catch (value) { setError(value instanceof Error ? value.message : t('common:errors.operationFailed')) }
    finally { setBusy('') }
  }
  ...
  async function rename(event: React.FormEvent) {
    ...
    try { const updated = await api.updateOrganization(space.id, name.trim()); setSpace(updated); setMessage(t('space.renamed')) }
    catch (value) { setError(value instanceof Error ? value.message : t('common:errors.operationFailed')) }
    finally { setBusy('') }
  }
