...
export default function MonitoringOverview() {
  ...
  return <main className="admin-content-page monitoring-page"><div className="admin-page-intro"><div><h1>{t('title')}</h1><p>{t('subtitle')}</p></div><div className="monitoring-actions"><span className={`monitor-auto-state ${value?.collector_stale ? 'stale' : ''} ${value && !value.automatic_collection ? 'paused' : ''}`}><i />{t(value && !value.automatic_collection ? 'overview.autoPaused' : value?.collector_stale ? 'overview.autoDelayed' : 'overview.autoRunning', { seconds: value?.interval_seconds || 60 })}</span><button className="icon-button" disabled={collecting} onClick={collect}>{collecting ? <span className="action-spinner"/> : <Icon name="analytics"/>}{t(collecting ? 'overview.collecting' : 'overview.collect')}</button></div></div><MonitoringNav />
    ...
    {detailKey && <MonitoringDetailDrawer metricKey={detailKey} ranges={value?.raw_ranges} onClose={()=>setDetailKey(null)}/>}
  </main>
}
