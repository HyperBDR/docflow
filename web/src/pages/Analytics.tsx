...
import type { Analytics as AnalyticsData, Demo, ShareLink, Tag } from '../types'
...
export default function Analytics() {
  ...
  const load = () => { setLoading(true); setError(''); api.analytics(id, from, to, tag ? [tag] : [],share).then(setData).catch(value => setError(value.message)).finally(() => setLoading(false)) }
  useEffect(() => { Promise.all([api.demo(id), api.tags(),api.shareLinks(id)]).then(([demoValue, tagValues,shareValues]) => { setDemo(demoValue); setTags(tagValues);setShares(shareValues) }).catch(value => setError(value.message)) }, [id])
  useEffect(load, [id, from, to, tag,share])
  ...
  return <main className="analytics-page">
    ...
    <section className="analytics-hero"><div><span className="analytics-icon"><Icon name="analytics" size={22} /></span><div><h1>{t('title')}</h1><p>{t('subtitle')}</p></div></div><div className="analytics-filters"><label>{t('from')}<input type="date" value={from} max={to} onChange={event => setFrom(event.target.value)} /></label><label>{t('to')}<input type="date" value={to} min={from} onChange={event => setTo(event.target.value)} /></label><label>{t('shareFilter')}<select value={share} onChange={event=>setShare(event.target.value)}><option value="">{t('allShares')}</option>{shares.map(item=><option key={item.id} value={item.id}>{item.name||t('unnamedShare')}</option>)}</select></label><label>{t('tagFilter')}<select value={tag} onChange={event => setTag(event.target.value)}><option value="">{t('allTags')}</option>{tags.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div></section>
    ...
    {!loading && data && tab === 'devices' && <section className="device-grid"><Distribution title={t('devices.os')} icon="device" items={data.devices.operating_systems} empty={t('devices.osEmpty')} /><Distribution title={t('devices.browser')} icon="layout" items={data.devices.browsers} empty={t('devices.browserEmpty')} /><Distribution title={t('devices.type')} icon="device" items={data.devices.device_types} empty={t('devices.typeEmpty')} /><Distribution title={t('devices.location')} icon="target" items={data.devices.locations} empty={t('devices.locationEmpty')} /><Distribution title={t('devices.source')} icon="layout" items={data.devices.sources} empty={t('devices.sourceEmpty')} /><Distribution title="UTM Source" icon="target" items={data.devices.utm_sources} empty={t('devices.utmEmpty')} /></section>}
