...
export default function AdminRecycle() {
  ...
  return <div className="admin-content-page">
    ...
    {selected && <><button className="admin-user-drawer-scrim" aria-label={t('common:actions.close')} onClick={() => setSelected(null)} /><aside className="admin-user-drawer monitor-detail-drawer recycle-detail-drawer" role="dialog" aria-modal="true" aria-labelledby="recycle-detail-title"><header className="admin-user-drawer-header"><div className="monitor-detail-identity recycle-detail-identity"><span className={selected.item_type}><Icon name={selected.item_type === 'user' ? 'user' : selected.item_type === 'team_space' ? 'users' : 'folder'} size={22} /></span><div><h2 id="recycle-detail-title">{selected.title}</h2><p>{t(`recycle.types.${selected.item_type}`)} · {selected.owner_email || '—'}</p><div className="admin-user-meta"><em>{t('recycle.details.deleted')}</em><small>{formatDate(selected.deleted_at, locale)}</small></div></div></div><button className="admin-user-drawer-close" aria-label={t('common:actions.close')} onClick={() => setSelected(null)}>×</button></header><div className="admin-user-drawer-body monitor-detail-body recycle-detail-body">
      {selected.item_type === 'resource' && <section className="monitor-detail-card recycle-resource-preview">{selected.thumbnail_url ? <img src={selected.thumbnail_url} alt="" /> : <div className="recycle-preview-placeholder"><Icon name="image" size={34} /></div>}<div><h3>{selected.title}</h3><p>{preview.description || t('recycle.details.noDescription')}</p></div></section>}
      <section className="monitor-detail-card"><header><div><strong>{t('recycle.details.summary')}</strong><small>{t('recycle.details.summaryHint')}</small></div></header><div className="recycle-detail-grid monitor-detail-card-scroll">
