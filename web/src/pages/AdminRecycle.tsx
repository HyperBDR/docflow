...
export default function AdminRecycle() {
  ...
  return <div className="admin-content-page">
    ...
    {selected && <><button className="admin-user-drawer-scrim" aria-label={t('common:actions.close')} onClick={() => setSelected(null)} /><aside className="admin-user-drawer recycle-detail-drawer" role="dialog" aria-modal="true" aria-labelledby="recycle-detail-title"><header className="admin-user-drawer-header"><div className="recycle-detail-identity"><span className={selected.item_type}><Icon name={selected.item_type === 'user' ? 'user' : selected.item_type === 'team_space' ? 'users' : 'folder'} size={22} /></span><div><h2 id="recycle-detail-title">{selected.title}</h2><p>{t(`recycle.types.${selected.item_type}`)} · {selected.owner_email || '—'}</p><div className="admin-user-meta"><em>{t('recycle.details.deleted')}</em><small>{formatDate(selected.deleted_at, locale)}</small></div></div></div><button className="admin-user-drawer-close" aria-label={t('common:actions.close')} onClick={() => setSelected(null)}>×</button></header><div className="admin-user-drawer-body">
      {selected.item_type === 'resource' && <section className="admin-user-section recycle-resource-preview">{selected.thumbnail_url ? <img src={selected.thumbnail_url} alt="" /> : <div className="recycle-preview-placeholder"><Icon name="image" size={34} /></div>}<div><h3>{selected.title}</h3><p>{selected.preview.description || t('recycle.details.noDescription')}</p></div></section>}
      <section className="admin-user-section"><div className="admin-user-section-title"><span><Icon name={selected.item_type === 'user' ? 'user' : selected.item_type === 'team_space' ? 'users' : 'list'} /></span><div><h3>{t('recycle.details.summary')}</h3><p>{t('recycle.details.summaryHint')}</p></div></div><div className="recycle-detail-grid">
        {selected.item_type === 'resource' && <><div><small>{t('recycle.details.teamSpace')}</small><strong>{selected.preview.organization_name || '—'}</strong></div><div><small>{t('recycle.details.status')}</small><strong>{t(`common:status.${selected.preview.status}`, { defaultValue: selected.preview.status || '—' })}</strong></div><div><small>{t('detail.steps')}</small><strong>{selected.preview.step_count || 0}</strong></div><div><small>{t('detail.views')}</small><strong>{selected.preview.views || 0}</strong></div><div><small>{t('detail.storage')}</small><strong>{bytes(selected.preview.storage_bytes, locale)}</strong></div><div><small>{t('detail.language')}</small><strong>{selected.preview.content_locale === 'en' ? 'English' : '简体中文'}</strong></div></>}
        {selected.item_type === 'user' && <><div><small>{t('recycle.details.email')}</small><strong>{selected.preview.email || selected.owner_email}</strong></div><div><small>{t('recycle.details.role')}</small><strong>{t(`roles.${selected.preview.role}`, { defaultValue: selected.preview.role || '—' })}</strong></div><div><small>{t('recycle.details.teams')}</small><strong>{selected.preview.team_count || 0}</strong></div><div><small>{t('recycle.details.resources')}</small><strong>{selected.preview.resource_count || 0}</strong></div><div><small>{t('detail.storage')}</small><strong>{bytes(selected.preview.storage_bytes, locale)}</strong></div></>}
        {selected.item_type === 'team_space' && <><div><small>{t('recycle.details.slug')}</small><code>{selected.preview.slug || '—'}</code></div><div><small>{t('recycle.details.owner')}</small><strong>{selected.preview.owner_name || selected.owner_email || '—'}</strong></div><div><small>{t('recycle.details.members')}</small><strong>{selected.preview.member_count || 0}</strong></div><div><small>{t('recycle.details.resources')}</small><strong>{selected.preview.resource_count || 0}</strong></div></>}
        <div><small>{t('recycle.details.createdAt')}</small><strong>{selected.preview.created_at ? formatDate(selected.preview.created_at, locale) : '—'}</strong></div>{selected.preview.updated_at && <div><small>{t('recycle.details.updatedAt')}</small><strong>{formatDate(selected.preview.updated_at, locale)}</strong></div>}
      </div></section>
      <section className="admin-user-section"><div className="admin-user-section-title warm"><span><Icon name="delete" /></span><div><h3>{t('recycle.details.deletion')}</h3><p>{t('recycle.details.deletionHint')}</p></div></div><div className="recycle-deletion-grid"><div><small>{t('recycle.details.deletedBy')}</small><strong>{selected.deleted_by_name || '—'}</strong></div><div><small>{t('recycle.details.deletedAt')}</small><strong>{formatDate(selected.deleted_at, locale)}</strong></div><div><small>{t('recycle.details.purgeAt')}</small><strong>{formatDate(selected.expires_at, locale)}</strong></div></div></section>
