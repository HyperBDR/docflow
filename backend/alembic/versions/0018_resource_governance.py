...
def upgrade() -> None:
    ...
    for name in ("created_by_id", "expires_at"):
        if name not in {item["name"] for item in inspector.get_indexes("share_tokens")}:
            op.create_index(f"ix_share_tokens_{name}", "share_tokens", [name])
    ...
    if "export_download_events" not in inspector.get_table_names():
        ...
        for name in ("export_job_id", "demo_id", "organization_id", "requested_by_id", "request_id", "source", "status", "created_at"):
            op.create_index(f"ix_export_download_events_{name}", "export_download_events", [name])
        op.create_index("ix_export_download_demo_created", "export_download_events", ["demo_id", "created_at"])
        op.create_index("ix_export_download_job_created", "export_download_events", ["export_job_id", "created_at"])
