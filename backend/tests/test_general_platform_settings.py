...
def test_public_config_is_empty_until_admin_configures_help_url(client):
    ...
    saved = client.patch("/api/admin/settings/general", json={"help_url": " https://docs.example.com/help "})
    assert saved.status_code == 200
    assert saved.json()["help_url"] == "https://docs.example.com/help"
    assert client.get("/api/platform/config").json() == {"help_url": "https://docs.example.com/help"}
...
def test_help_url_can_be_cleared_and_rejects_unsafe_urls(client):
    ...
    for invalid in ("javascript:alert(1)", "//docs.example.com", "https://user:secret@docs.example.com", "https://docs.example.com\ninvalid", "https://docs.example.com:bad"):
        assert client.patch("/api/admin/settings/general", json={"help_url": invalid}).status_code == 422
    assert client.patch("/api/admin/settings/general", json={"help_url": "https://docs.example.com"}).status_code == 200
